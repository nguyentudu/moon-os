# Moon OS

Moon OS is an awareness infrastructure for personal continuity.

## Moon OS Architecture

LAW

- [Personal Continuity Manifest](PERSONAL_CONTINUITY_MANIFEST.md)
- [Trust Governance](TRUST_GOVERNANCE.md)
- [Attention Constitution](ATTENTION_CONSTITUTION.md)

↓

KNOWLEDGE

- [Continuity Knowledge Base](docs/knowledge/knowledge_base_index.md)

↓

GRAPH

- [Continuity Graph](docs/graph/continuity_graph_schema_v1.md)

↓

METRICS

- [Awareness Metrics](docs/metrics/awareness_metrics_v1.md)

↓

REASONING

- [Continuity Director](docs/reasoning/continuity_director_charter_v1.md)

↓

AWARENESS / RELEVANCE QA

- [Awareness / Relevance QA](docs/reasoning/awareness/awareness_qa_charter_v1.md)

↓

TRUST & CONSENT QA

- [Trust & Consent QA](docs/reasoning/trust/trust_qa_charter_v1.md)

↓

TASK WRITER

- [Moon Task Writer](docs/reasoning/task_writer/moon_task_writer_charter_v1.md)

↓

TODAY SURFACE CONTRACT

- [Moon Today Surface Contract](docs/today/moon_today_surface_contract_v1.md)

↓

TODAY RUNTIME MVP

- [Moon Today Runtime MVP](apps/moon_today_runtime/README.md)

## Constitutional Validation Layer

CONSTITUTIONAL CORE

↓

VALIDATION

- [Real User Scenario Simulation](docs/validation/validation_framework_v1.md)

↓

BENCHMARK

- [Benchmark Execution Review](docs/validation/execution/BENCHMARK_EXECUTION_REVIEW.md)

↓

AUDIT

- [Constitutional Audit Review](docs/validation/audit/CONSTITUTIONAL_AUDIT_REVIEW.md)

↓

VALIDATED INTELLIGENCE REPORT

- [Validated Intelligence Report](docs/validation/validated/validated_intelligence_report_v1.md)

↓

TODAY MVP PROOF

- [Moon Today MVP Proof](apps/moon_today_proof/README.md)

↓

HUMAN EVALUATION REPORT

- [Human Evaluation Report](docs/evaluation/human/HUMAN_EVALUATION_REPORT_V1.md)

↓

DATA INGESTION PROTOTYPE

↓

INTERNAL PILOT

↓

EXTERNAL PILOT

## Research Quality Track

BOUNDARY VALIDATION

- [Boundary Validation Review](docs/decision_memory/boundary_validation_review/BOUNDARY_VALIDATION_REVIEW.md)

↓

ROUND 3

↓

DEFINITION DECISION

↓

DECISION EXPANSION

## Economic Validation Track

## Current Evidence Status

Decision Memory Foundation ✓

Reliability Evidence ✓

Boundary Validation ✓

Economic Instrumentation ✓

Raw TTC Evidence ✓

Early TTC Signal ✓

Early Quality Signal âœ“

↓

Shared Retrieval âœ“

↓

Shared Understanding Study

↓

Shared Judgement Signal ?

â†“

Time To Contribution Pilot ?

â†“

Team Continuity Pilot ?

## Roadmap

ECONOMIC INSTRUMENTATION

- [Economic Instrumentation](docs/economic_validation/ECONOMIC_INSTRUMENTATION_REVIEW.md)

↓

TTC STUDY

- [Time To Context Measurement](docs/economic_validation/time_to_context_measurement_v1.md)

↓

RAW TTC EVIDENCE

- [TTC Study Results Review](docs/economic_validation/ttc_study_results/TTC_STUDY_RESULTS_REVIEW.md)

↓

EARLY TTC SIGNAL ANALYSIS

- [Early TTC Signal Summary](docs/economic_validation/ttc_study_results/early_ttc_signal_summary_v1.md)

↓

ANSWER QUALITY REVIEW

- [Blinded TTC Answer Quality Review](docs/economic_validation/ttc_answer_quality_review/ANSWER_QUALITY_REVIEW.md)

↓

SHARED UNDERSTANDING REVIEW

- [Shared Understanding Study](docs/economic_validation/shared_understanding_study/SHARED_UNDERSTANDING_STUDY_REVIEW.md)

↓

CONTRIBUTOR STUDY

- [Time To Contribution Measurement](docs/economic_validation/time_to_contribution_measurement_v1.md)

↓

CONTINUITY LOOP V1

↓

ONBOARDING STUDY

- [Economic Study Design](docs/economic_validation/economic_study_design_v1.md)

↓

REAL TEAM TRIAL
